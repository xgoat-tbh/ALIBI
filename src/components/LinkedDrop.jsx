...
function LinkedDrop({ currentPrompt, currentRound, totalRounds, phaseTimer, mySubmission, onSubmitWord }) {
  ...
  useEffect(() => { setInput(mySubmission || ''); setSubmitted(!!mySubmission); }, [mySubmission, currentPrompt]);
  useEffect(() => { if (!submitted) inputRef.current?.focus(); }, [currentPrompt, submitted]);
