  ...
  return (
    <div key={chat.id} style={{
      ...
      <div style={{
        ...
        fontSize: '0.8rem',
        lineHeight: '1.4',
        background: isSys ? 'rgba(255, 255, 255, 0.02)' : isMe ? 'var(--accent-primary)' : 'var(--bg-raised)',
      ...
      <div style={{
        ...
        <form onSubmit={handleSendChat} style={{ padding: '12px 16px', borderTop: '1px solid var(--border-glass)', backgroundColor: 'var(--bg-raised)' }}>
          <div style={{ display: 'flex', gap: '8px' }}>
            <input type="text" placeholder="Type a message..." value={chatInput} onChange={(e) => setChatInput(e.target.value)} style={{ padding: '8px 12px', fontSize: '0.85rem', flex: 1 }} autoFocus />
  ...
  return (
    <aside style={{ display: 'flex', flexDirection: 'column', background: 'rgba(7, 8, 10, 0.5)', borderLeft: 'none' }}>
      ...
      <form onSubmit={handleSendChat} style={{ padding: '16px', borderTop: '1px solid var(--border-glass)', backgroundColor: 'var(--bg-raised)' }}>
        <div style={{ display: 'flex', gap: '8px' }}>
          <input type="text" placeholder="Type a message..." value={chatInput} onChange={(e) => setChatInput(e.target.value)} style={{ padding: '8px 12px', fontSize: '0.8rem' }} />
