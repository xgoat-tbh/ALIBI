...
import { MessageSquare, MessageCircle, LogOut, Wifi, WifiOff, AlertCircle, Volume2, VolumeX } from 'lucide-react';
...
function GamePage({ onOpenRules }) {
  ...
  const { errorMsg, showChatPanel, mobileChatOpen, isMuted, setMuted } = uiStore;
  ...
  return (
    <div className="game-container">
      {!isConnected && (
        <div role="alert" style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 9999, padding: '10px 16px', background: 'rgba(220, 38, 38, 0.15)', borderBottom: '1px solid rgba(220, 38, 38, 0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', color: 'var(--color-danger)', fontSize: '0.85rem', fontWeight: '600', backdropFilter: 'blur(8px)' }}>
          <WifiOff size={16} /><span>Connection lost. Reconnecting...</span>
        </div>
      )}

      {/* Header */}
      <header style={{ padding: isMobile ? '10px 12px' : '14px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', zIndex: 10, background: 'rgba(10, 13, 15, 0.6)', flexWrap: 'wrap', gap: '6px', minHeight: isMobile ? 'auto' : '58px' }}>
        ...
        <div className="flex-row" style={{ gap: isMobile ? '6px' : '10px', flexShrink: 0 }}>
          ...
          <button onClick={() => setMuted(!isMuted)} className="btn-secondary" style={{ padding: isMobile ? '4px 8px' : '6px 12px', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '4px' }} aria-label={isMuted ? 'Unmute sounds' : 'Mute sounds'}>
      ...
      {/* Main area */}
      <div style={{ display: 'grid', gridTemplateColumns: (showChatPanel && !isMobile) ? '1fr 320px' : '1fr', flex: 1, height: isMobile ? 'calc(100dvh - 48px - 50px)' : 'calc(100vh - 58px - 62px)', overflow: 'hidden', position: 'relative' }}>
      ...
      <footer style={{ padding: isMobile ? '6px 12px' : '10px 24px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(10, 13, 15, 0.4)', height: isMobile ? 'auto' : '62px', minHeight: isMobile ? '42px' : '62px' }}>
        <div style={{ display: 'flex', gap: isMobile ? '6px' : '12px', overflowX: 'auto', flex: 1, alignItems: 'center', paddingBottom: isMobile ? '4px' : 0 }}>
          {!isMobile && <span style={{ fontSize: '0.7rem', fontWeight: '700', textTransform: 'uppercase', color: 'var(--text-muted)', borderRight: '1px solid var(--border-glass)', paddingRight: '12px', letterSpacing: '0.08em', whiteSpace: 'nowrap' }}>Players</span>}
          {players.map((p) => {
            const isMe = p.id === playerId;
            return (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: isMobile ? '3px' : '6px', padding: isMobile ? '2px 6px' : '4px 10px', borderRadius: 'var(--radius-button)', whiteSpace: 'nowrap', flexShrink: 0 }}>
                <span style={{ fontSize: isMobile ? '0.65rem' : '0.8rem', fontWeight: isMe ? '700' : '500', color: isMe ? '#ffffff' : 'var(--text-secondary)' }}>
                  {p.name}
                </span>
                <span className="font-tech" style={{ fontSize: isMobile ? '0.65rem' : '0.75rem', fontWeight: '700', color: 'var(--accent-primary)' }}>
                  {p.score}
                </span>
              </div>
            );
